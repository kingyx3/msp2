...
describe('Check AccountStack & ensure navigation works across bookings & spaces', () => {
  ...
  it('Attempt host log tap', async () => {
    ...
    if (spaceLogTypeArray.includes(hostLogLabel)) {
      ...
      await waitFor(element(by.id("booking-detail-scroll-view"))).toBeVisible().withTimeout(20000) // Not visible for past bookings, visible up to booking start (host), visible up to 2 days after booking end (host)
      await waitFor(element(by.id("booking-title-price"))).toBeVisible().withTimeout(20000) // Not visible for past bookings, visible up to booking start (host), visible up to 2 days after booking end (host)
